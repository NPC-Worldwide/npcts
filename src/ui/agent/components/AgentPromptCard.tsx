...
export const AgentPromptCard: React.FC<AgentPromptCardProps> = ({ promptData, onResolve }) => {
          ...
          {options.map((opt, i) => {
            ...
            return (
              <button
                ...
                disabled={responded}
                onClick={() => !responded && onResolve({ choice: opt, index: i })}
              >
                {isSelected && '✓ '}{opt}
              </button>
            );
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            disabled={responded}
            onClick={() => !responded && onResolve({ confirmed: true })}
          >
          ...
          <button
            ...
            disabled={responded}
            onClick={() => !responded && onResolve({ confirmed: false })}
          >
      ...
      <div style={cardStyle}>
        ...
        {responded ? (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              style={btnPrimary}
              onClick={() => textValue.trim() && onResolve({ text: textValue.trim() })}
            >
              Send
            </button>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {!responded && (
            <button
              ...
              onClick={() => onResolve({ fields: formValues })}
            >
              Submit
            </button>
          )}
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={msgStyle}>{promptData.message}</div>
      {!responded && (
        <button style={btnBase} onClick={() => onResolve({ dismissed: true })}>
          OK
        </button>
      )}
      {responded && (
