...
import { latLonToVector } from "../lib/geo.js";
...
export function CityLabels() {
  const atMaxZoom = useGlobeStore((state) => state.globeView?.atMaxZoom ?? false);
  ...
  useFrame(({ camera }) => {
    ...
    labels.forEach((city, index) => {
      ...
      label.style.display =
        !atMaxZoom && city.tier <= maxTier && isFacingCamera ? "block" : "none";
    });
  });
