...
const stubFileApi: DocumentFileApi = {
    readFileBuffer: () => Promise.reject(new Error('No fileApi provided to DocxViewer')),
    readFileContent: () => Promise.reject(new Error('No fileApi provided to DocxViewer')),
    writeFileContent: () => Promise.reject(new Error('No fileApi provided to DocxViewer')),
    writeFileBuffer: () => Promise.reject(new Error('No fileApi provided to DocxViewer')),
};
...
export interface DocxViewerHandle {
    readDocumentContent: (opts?: { format?: 'text' | 'html' }) => any;
    evalDocument: (code: string) => any;
    writeDocumentContent: (html: string) => any;
    insertDocumentContent: (html: string, position?: 'cursor' | 'end' | 'start') => any;
    formatDocument: (command: string, value?: string) => any;
    insertDocumentTable: (rows: number, cols: number) => any;
    findInDocument: (searchText: string) => any;
    replaceInDocument: (search: string, replacement: string, all?: boolean) => any;
    saveDocument: () => Promise<any>;
    getDocumentStats: () => any;
    exportDocumentAs: (format: 'html' | 'markdown') => Promise<any>;
}

export const DocxViewer = forwardRef<DocxViewerHandle, DocxViewerProps>(({ filePath, fileApi = stubFileApi, onSave: onSaveCallback, onClose, onDirtyChange, cacheKey }, ref) => {
    ...
    useEffect(() => {
        const loadDocx = async () => {
            ...
            try {
                ...
                const rawBuffer = await fileApi.readFileBuffer(filePath);
                ...
                const raw = rawBuffer as any;
                const buffer = rawBuffer instanceof Uint8Array ? rawBuffer :
                               raw?.data ? new Uint8Array(raw.data) :
                               raw?.type === 'Buffer' ? new Uint8Array(raw.data) :
                               new Uint8Array(rawBuffer ? new Uint8Array(rawBuffer) : []);
                ...
                if (isZip) {
                    ...
                    const response = fileApi.convertDocxToHtml ? await fileApi.convertDocxToHtml(filePath) : { html: '', error: 'DOCX conversion not available' } as any;
                    ...
                    const textContent = await fileApi.readFileContent(filePath);
                    const content = textContent || '<p><br></p>';
...
${htmlContent}
        ...
        await fileApi.writeFileContent(newPath, html);
    ...
    const exportAsMarkdown = useCallback(async () => {
        ...
        await fileApi.writeFileContent(newPath, text);
        alert('Exported to ' + newPath);
    ...
    useImperativeHandle(ref, () => ({
        readDocumentContent: (opts) => {
            const format = opts?.format || 'text';
            if (format === 'html') {
                return { success: true, content: editorRef.current?.innerHTML || '', format: 'html', filePath };
            }
            return { success: true, content: editorRef.current?.innerText || '', format: 'text', stats: documentStats, filePath };
        },
        evalDocument: (code) => {
            try {
                const fn = new Function('ctx', code);
                const result = fn({ html: editorRef.current?.innerHTML || '', text: editorRef.current?.innerText || '', editorEl: editorRef.current });
                if (result?.html !== undefined && editorRef.current) { editorRef.current.innerHTML = result.html; handleInput(); }
                setHasChanges(true);
                return { success: true, ...(result || {}) };
            } catch (err: any) { return { success: false, error: err.message }; }
        },
        writeDocumentContent: (html) => {
            if (editorRef.current) { editorRef.current.innerHTML = html; handleInput(); setHasChanges(true); }
            return { success: true };
        },
        insertDocumentContent: (html, position) => {
            if (!editorRef.current) return { success: false, error: 'Editor not ready' };
            if (position === 'end') editorRef.current.innerHTML += html;
            else if (position === 'start') editorRef.current.innerHTML = html + editorRef.current.innerHTML;
            else insertAtCursor(html);
            handleInput(); setHasChanges(true);
            return { success: true };
        },
        ...
        insertDocumentTable: (rows, cols) => { insertTable(rows, cols); return { success: true, rows, cols }; },
        findInDocument: (searchText) => {
            const content = editorRef.current?.innerText || '';
            const regex = new RegExp(searchText, 'gi');
            const matches = content.match(regex);
            return { success: true, matchCount: matches?.length || 0, searchText };
        },
        replaceInDocument: (search, replacement, all) => {
            if (!editorRef.current) return { success: false, error: 'Editor not ready' };
            const content = editorRef.current.innerHTML;
            const regex = new RegExp(search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), all ? 'gi' : 'i');
            const newContent = content.replace(regex, replacement);
            const changed = newContent !== content;
            if (changed) { editorRef.current.innerHTML = newContent; handleInput(); setHasChanges(true); }
            return { success: true, changed };
        },
        saveDocument: async () => { await saveDocument(); return { success: true }; },
        getDocumentStats: () => { return { success: true, ...documentStats, filePath }; },
        exportDocumentAs: async (format) => {
            if (format === 'html') await exportAsHtml();
            else if (format === 'markdown') await exportAsMarkdown();
            return { success: true, format };
        },
    }), [filePath, documentStats, handleInput, insertAtCursor, execCommand, insertTable, saveDocument, exportAsHtml, exportAsMarkdown]);
        ...
        <div className="h-full flex flex-col theme-bg-secondary overflow-hidden">
            ...
            >
                ...
                <div className="flex items-center gap-1">
                    ...
                    {onClose && (
                        <button onClick={() => onClose()} className="p-1.5 theme-hover rounded" title="Close">
                            <X size={14} />
                        </button>
                    )}
                </div>
