import { getFileName, DocumentFileApi } from '../../../core/files';
...
const stubFileApi: DocumentFileApi = {
    readFileBuffer: () => Promise.reject(new Error('No fileApi provided to CsvViewer')),
    readFileContent: () => Promise.reject(new Error('No fileApi provided to CsvViewer')),
    writeFileContent: () => Promise.reject(new Error('No fileApi provided to CsvViewer')),
    writeFileBuffer: () => Promise.reject(new Error('No fileApi provided to CsvViewer')),
};

export interface CsvViewerProps {
    filePath: string;
    fileApi?: DocumentFileApi;
    onSave?: () => void;
    onClose?: () => void;
    onDirtyChange?: (dirty: boolean) => void;
    /** Optional cache key (defaults to filePath) */
    cacheKey?: string;
}

export interface CsvViewerHandle {
    readSpreadsheetData: (opts?: { maxRows?: number; includeStats?: boolean }) => any;
    evalSpreadsheet: (code: string) => any;
    updateSpreadsheetCell: (row: number, col: number, value: any) => any;
    updateSpreadsheetCells: (updates: { row: number; col: number; value: any }[]) => any;
    updateSpreadsheetHeader: (col: number, value: string) => any;
    addSpreadsheetRow: (index?: number) => any;
    deleteSpreadsheetRow: (index: number) => any;
    addSpreadsheetColumn: (name?: string) => any;
    deleteSpreadsheetColumn: (col: number) => any;
    sortSpreadsheet: (col: number, direction: 'asc' | 'desc') => any;
    filterSpreadsheet: (col: number, value: string) => any;
    clearSpreadsheetFilters: () => any;
    getSpreadsheetColumnStats: (col: number) => any;
    saveSpreadsheet: () => Promise<any>;
    exportSpreadsheet: (format: 'csv' | 'json' | 'xlsx') => Promise<any>;
    switchSpreadsheetSheet: (sheetName: string) => any;
}

export const CsvViewer = forwardRef<CsvViewerHandle, CsvViewerProps>(({
    filePath,
    fileApi = stubFileApi,
    ...
    // Sync edited data to global cache so edits survive component remount
    useEffect(() => {
        const fp = cacheKey || filePath;
    ...
    }, [headers, data, hasChanges, filePath, cacheKey]);
    ...
    const loadSpreadsheet = useCallback(async () => {
        ...
        try {
            if (isXlsx) {
                const buffer = await fileApi.readFileBuffer(filePath);
                let wb;
                ...
                // Try specialized CSV reader, fall back to reading text + XLSX parse
                let csvHeaders: any[] = ['Column 1'];
                let csvRows: any[][] = [[]];
                if (fileApi.readCsvContent) {
                    const response = await fileApi.readCsvContent(filePath);
                    if (response.error) throw new Error(response.error);
                    // Parse with XLSX from text content
                    const wb = XLSX.read(response.content, { type: 'string' });
                    const sheet = wb.Sheets[wb.SheetNames[0]];
                    const jsonData = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '' });
                    csvHeaders = jsonData[0] as any[] || ['Column 1'];
                    csvRows = jsonData.slice(1).length > 0 ? jsonData.slice(1) as any[][] : [[]];
                } else {
                    const content = await fileApi.readFileContent(filePath);
                ...
                setData(csvRows);
                csvDataCache.set(filePath, { headers: csvHeaders, data: csvRows });
            }
            setHasChanges(false);
            setError(null);
    ...
    const saveSpreadsheet = useCallback(async () => {
        try {
            if (isXlsx && workbook) {
                ...
                await fileApi.writeFileBuffer(filePath, new Uint8Array(s2ab(wbout)));
            } else {
                const csvContent = [
                    headers.join(','),
                    ...data.map(row => 
                ...
                await fileApi.writeFileContent(filePath, csvContent);
            }
            setHasChanges(false);
            onSaveCallback?.();
        } catch (err) {
    ...
    }, [isXlsx, workbook, headers, data, activeSheet, filePath, fileApi, onSaveCallback]);
    ...
    const exportData = useCallback(async (format: 'csv' | 'json' | 'xlsx') => {
        try {
            ...
            if (format === 'csv') {
                ...
                await fileApi.writeFileContent(`${baseName}_export.csv`, csvContent);
            } else if (format === 'json') {
                ...
                await fileApi.writeFileContent(`${baseName}_export.json`, JSON.stringify(jsonData, null, 2));
            } else if (format === 'xlsx') {
                const wb = XLSX.utils.book_new();
                const ws = XLSX.utils.aoa_to_sheet([headers, ...data]);
                XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
                const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'binary' });
                const s2abLocal = (s: string) => {
                    const buf = new ArrayBuffer(s.length);
                    const view = new Uint8Array(buf);
                    for (let i = 0; i < s.length; i++) view[i] = s.charCodeAt(i) & 0xFF;
                    return buf;
                };
                await fileApi.writeFileBuffer(`${baseName}_export.xlsx`, new Uint8Array(s2abLocal(wbout)));
            }
        } catch (err) {
            setError(err.message);
        }
    ...
    // Refs for imperative handle (avoid stale closures)
    const dataRef = useRef(data);
    const headersRef = useRef(headers);
    useEffect(() => { dataRef.current = data; }, [data]);
    useEffect(() => { headersRef.current = headers; }, [headers]);

    // Expose imperative handle for parent (Studio Actions)
    useImperativeHandle(ref, () => ({
        readSpreadsheetData: (opts) => {
            const limit = opts?.maxRows || 500;
            const d = dataRef.current;
            const h = headersRef.current;
            const result: any = {
                success: true, headers: [...h], data: d.slice(0, limit),
                rowCount: d.length, columnCount: h.length,
                truncated: d.length > limit, activeSheet, sheetNames, filePath,
            };
            if (opts?.includeStats) {
                result.columnStats = h.map((hdr: string, i: number) => ({ header: hdr, ...getColumnStats(i) }));
            }
            return result;
        },
        evalSpreadsheet: (code) => {
            try {
                const fn = new Function('ctx', code);
                const result = fn({ headers: [...headersRef.current], data: dataRef.current.map((r: any[]) => [...r]), XLSX });
                if (result?.headers) setHeaders(result.headers);
                if (result?.data) { setData(result.data); addToHistory(result.data, result.headers || headersRef.current); }
                setHasChanges(true);
                return { success: true, rowCount: result?.data?.length ?? dataRef.current.length, columnCount: result?.headers?.length ?? headersRef.current.length };
            } catch (err: any) { return { success: false, error: err.message }; }
        },
        updateSpreadsheetCell: (row, col, value) => { updateCell(row, col, value); return { success: true, row, col, value }; },
        updateSpreadsheetCells: (updates) => {
            setData(prev => {
                const newData = prev.map((r: any[]) => [...r]);
                for (const u of updates) { if (!newData[u.row]) newData[u.row] = new Array(headersRef.current.length).fill(''); newData[u.row][u.col] = u.value; }
        ...
        updateSpreadsheetHeader: (col, value) => { updateHeader(col, value); return { success: true, col, value }; },
        addSpreadsheetRow: (index) => { addRow(index); return { success: true, rowCount: dataRef.current.length + 1 }; },
        deleteSpreadsheetRow: (index) => { deleteRow(index); return { success: true }; },
        addSpreadsheetColumn: (name) => { addColumn(); if (name) updateHeader(headersRef.current.length, name); return { success: true, columnCount: headersRef.current.length + 1 }; },
        deleteSpreadsheetColumn: (col) => { deleteColumn(col); return { success: true }; },
        sortSpreadsheet: (col, direction) => { setSortConfig({ column: col, direction }); return { success: true, column: col, direction }; },
        filterSpreadsheet: (col, value) => { setFilters(prev => ({ ...prev, [col]: value })); return { success: true, column: col, filter: value }; },
        clearSpreadsheetFilters: () => { clearFilters(); return { success: true }; },
        getSpreadsheetColumnStats: (col) => { return { success: true, header: headersRef.current[col], ...getColumnStats(col) }; },
        saveSpreadsheet: async () => { await saveSpreadsheet(); return { success: true }; },
        exportSpreadsheet: async (format) => { await exportData(format); return { success: true, format }; },
        switchSpreadsheetSheet: (sheetName) => {
            if (workbook && sheetNames.includes(sheetName)) { switchSheet(sheetName); return { success: true, sheet: sheetName }; }
            return { success: false, error: `Sheet not found: ${sheetName}. Available: ${sheetNames.join(', ')}` };
        },
    }), [activeSheet, sheetNames, filePath, workbook, updateCell, updateHeader, addRow, deleteRow, addColumn, deleteColumn, getColumnStats, saveSpreadsheet, exportData, clearFilters, addToHistory, switchSheet]);
