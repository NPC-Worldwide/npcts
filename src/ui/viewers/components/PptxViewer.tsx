...
const stubFileApi: DocumentFileApi = {
    readFileBuffer: () => Promise.reject(new Error('No fileApi provided to PptxViewer')),
    readFileContent: () => Promise.reject(new Error('No fileApi provided to PptxViewer')),
    writeFileContent: () => Promise.reject(new Error('No fileApi provided to PptxViewer')),
    writeFileBuffer: () => Promise.reject(new Error('No fileApi provided to PptxViewer')),
};
...
export interface PptxViewerHandle {
    readPresentation: () => any;
    readSlide: (slideIndex?: number) => any;
    evalPresentation: (code: string) => any;
    goToSlide: (slideIndex: number) => any;
    updateSlideText: (shapeIndex: number, text: string, slideIndex?: number) => any;
    addPresentationSlide: () => any;
    deletePresentationSlide: (slideIndex?: number) => any;
    duplicatePresentationSlide: (slideIndex?: number) => any;
    setPresentationSlideBackground: (color: string) => any;
    addPresentationShape: (shapeType: string, color?: string) => any;
    savePresentation: () => Promise<any>;
}

export const PptxViewer = forwardRef<PptxViewerHandle, PptxViewerProps>(({ filePath, fileApi = stubFileApi, onSave: onSaveCallback, onClose, onDirtyChange, cacheKey }, ref) => {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      await fileApi.writeFileBuffer(filePath, output);
      setHasChanges(false);
      onSaveCallback?.();
    } catch (e: any) {
  ...
  useImperativeHandle(ref, () => ({
    readPresentation: () => {
      ...
      return {
        success: true, slideCount: s.length, currentSlideIndex: idxRef.current,
        slides: s.map((slide, i) => ({
          index: i, name: slide.name, shapeCount: slide.shapes.length,
          textContent: slide.shapes.filter(sh => sh.type === 'text' && sh.paras)
            .map(sh => sh.paras!.map(p => { const div = document.createElement('div'); div.innerHTML = p.html; return div.innerText; }).join('\n')).join(' | '),
          background: slide.background,
        })),
        filePath, hasChanges,
      };
    },
    readSlide: (slideIndex) => {
      const si = slideIndex ?? idxRef.current;
      const slide = slidesRef.current[si];
      if (!slide) return { success: false, error: `Slide ${si} not found. Total: ${slidesRef.current.length}` };
      return {
        success: true, index: si,
        shapes: slide.shapes.map((sh, i) => ({
          index: i, type: sh.type, position: sh.xfrm,
          text: sh.paras?.map(p => { const div = document.createElement('div'); div.innerHTML = p.html; return div.innerText; }).join('\n'),
          fillColor: sh.fillColor, shapeType: sh.shapeType, name: sh.name,
        })),
        background: slide.background,
      };
    },
    evalPresentation: (code) => {
      try {
        const fn = new Function('ctx', code);
        const result = fn({ slides: JSON.parse(JSON.stringify(slidesRef.current)), currentIndex: idxRef.current });
        if (result?.slides) { setSlides(result.slides); setHasChanges(true); }
    ...
    goToSlide: (slideIndex) => {
      if (slideIndex < 0 || slideIndex >= slidesRef.current.length) return { success: false, error: `Invalid index: ${slideIndex}` };
      setIdx(slideIndex); return { success: true, slideIndex };
    },
    updateSlideText: (shapeIndex, text, slideIndex) => {
      const si = slideIndex ?? idxRef.current;
      setSlides(prev => {
        const next = [...prev]; const s = { ...next[si] }; const shapes = [...s.shapes]; const sh = { ...shapes[shapeIndex] };
        if (sh.paras && sh.paras.length > 0) {
          const paras = [...sh.paras];
          paras[0] = { ...paras[0], html: text.replace(/</g, '&lt;').replace(/>/g, '&gt;') };
          sh.paras = [paras[0]];
        }
        shapes[shapeIndex] = sh; s.shapes = shapes; next[si] = s; return next;
      });
      setHasChanges(true); return { success: true };
    },
    addPresentationSlide: () => { addSlide(); return { success: true, slideCount: slidesRef.current.length + 1 }; },
    deletePresentationSlide: (slideIndex) => {
      const si = slideIndex ?? idxRef.current;
      if (slidesRef.current.length <= 1) return { success: false, error: 'Cannot delete the only slide' };
      setSlides(prev => prev.filter((_, i) => i !== si)); setIdx(Math.max(0, si - 1)); setHasChanges(true);
      return { success: true };
    },
    duplicatePresentationSlide: (slideIndex) => {
      const si = slideIndex ?? idxRef.current;
      const slide = slidesRef.current[si];
      if (!slide) return { success: false, error: `Slide ${si} not found` };
      const cloned = { ...slide, shapes: slide.shapes.map(s => ({ ...s, paras: s.paras?.map(p => ({ ...p })) })) };
      setSlides(prev => { const next = [...prev]; next.splice(si + 1, 0, cloned); return next; });
      setIdx(si + 1); setHasChanges(true);
      return { success: true, slideCount: slidesRef.current.length + 1 };
    },
    setPresentationSlideBackground: (color) => { setSlideBackground(color); return { success: true, color }; },
    addPresentationShape: (shapeType, color) => { addShape(shapeType, color || '#4285f4'); return { success: true, shapeType }; },
    savePresentation: async () => { await save(); return { success: true }; },
  }), [filePath, hasChanges, addSlide, addShape, setSlideBackground, save]);
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="px-3 py-2 border-b theme-border theme-bg-secondary flex items-center justify-between"
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => onClose?.()} className="p-1.5 theme-hover rounded-full" title="Close"><X size={14} /></button>
        </div>
      </div>
